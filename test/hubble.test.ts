...
test("keeps Vault operations inside the root and protects symlinks", async () => {
  ...
  if (symlinkEscape.status === "error") {
    expect(symlinkEscape.error._tag).toBe("VaultPathError");
  }
  ...
  if (htmlSymlinkEscape.status === "error") {
    expect(htmlSymlinkEscape.error._tag).toBe("VaultPathError");
  }
...
test("atomically edits the canonical target of a symlink that stays inside the vault", async () => {
  ...
  if (edited.status === "ok") {
    expect(edited.value.relative).toBe("target.md");
  }
...
test("rejects a missing vault root replaced by a symlink before creation", async () => {
  ...
  if (created.status === "error") {
    ...
    if (created.error._tag === "VaultPathError") {
      expect(created.error.reason).toBe("symlink-escape");
    }
  }
...
test("uses exact optional filenames independently from titles and rejects unsafe or conflicting names", async () => {
  ...
  for (const filename of ["../escape.md", "nested/escape.md", "nested\\escape.md", "/absolute.md", "note.txt"]) {
    ...
    if (invalid.status === "error") {
      ...
      if (invalid.error._tag === "NoteValidationError") {
        expect(invalid.error.reason).toBe("filename");
      }
    }
  }
  ...
  if (mismatch.status === "error") {
    ...
    if (mismatch.error._tag === "NoteValidationError") {
      expect(mismatch.error.reason).toBe("format");
    }
  }
});
...
test("rejects overlapping duplicate exact edit matches without changing the note", async () => {
  ...
  if (edited.status === "error") {
    ...
    if (edited.error._tag === "EditValidationError") {
      expect(edited.error.reason).toBe("duplicate");
    }
  }
...
test("discovers, reads, searches, and edits Markdown and HTML notes", async () => {
  ...
  if (read.status === "ok") {
    expect(read.value.content).toBe("<main>HTML match</main>");
  }
  ...
  if (unsupported.status === "error") {
    ...
    if (unsupported.error._tag === "VaultPathError") {
      expect(unsupported.error.reason).toBe("unsupported-note-format");
    }
  }
});
...
test("uses local config over global config and the CLI as an escape hatch", async () => {
  ...
  try {
    ...
    if (local.status === "ok") {
      expect(local.value).toBe(resolve(process.cwd(), "local-vault"));
    }
    ...
    if (cli.status === "ok") {
      expect(cli.value).toBe(resolve(process.cwd(), "cli-vault"));
    }
  } finally {
...
test("ignores untrusted local config and requires configuration", async () => {
  ...
  try {
    ...
    if (trusted.status === "error") {
      expect(trusted.error._tag).toBe("ConfigParseError");
    }
    ...
    if (missing.status === "error") {
      expect(missing.error._tag).toBe("VaultNotConfiguredError");
    }
  } finally {
    if (originalAgentDir === undefined) {
      delete process.env.PI_CODING_AGENT_DIR;
